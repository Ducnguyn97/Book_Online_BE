...
public interface BookRepository extends JpaRepository<Book, Long> {
    ...
    """)
    List<Book> findRelatedBooksByGenreIds(@Param("bookId") Long bookId,
    ...
    @Query(value = "SELECT * FROM books WHERE is_deleted = false ORDER BY sold_quantity_book DESC LIMIT 10",
            nativeQuery = true)
...
//like concat('%',:keyword,'%') lay tat ca cac ky tu co trong keyword
    @Query("""
        ...
        """)
    Page<Book> findAllBookByAuthorBook(@Param("authorName") String authorName,
                                       Pageable pageable);
